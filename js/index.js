...
function buildTaskCard(task, index) {
    ...
    const dueStatus = task.status === "completed" ? "none" : getDueStatus(task.dueDate);
    ...
    return `<div
        ...
        : ""}
              ...
              <div class="flex items-center gap-3 text-xs text-slate-400 pb-3 mb-3 border-b border-slate-100">
                <div class="flex items-center gap-1.5 ${dueStatus === "overdue"
        ? "text-red-500"
        : dueStatus === "due-soon"
            ? "text-orange-500"
            : ""}">
...
document
    ...
    if (!selectedTask)
        return;
    if (getDueStatus(selectedTask.dueDate) === "overdue" &&
        newStatus === "completed") {
        Swal.fire({
